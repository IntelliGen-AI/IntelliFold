...
# This file is a copy of AlphaFold 3's `run_alphafold.py` (Apache License 2.0)
# modified by IntelliGen-AI in 2026. Changes vs. the upstream file:
#   * import `intellifold.patches`;
#   * in `make_model_config`, after `model.Model.Config()`, apply two env-gated
#     IntelliFold patches (Fourier-embedding injection + full_fat config widening)
#     so the stock AF3 JAX network loads IntelliFold-v2 weights;
#   * add a `--work_queue_dir` flag: an optional local work queue so one worker
#     per GPU can divide an --input_dir across this machine's GPUs with no overlap
#     and resume (see `intellifold.parallel`). Off unless the flag is set;
#   * change the default --flash_attention_implementation from 'triton' to 'cudnn'.
# The patches are no-ops unless INTFOLD_FOURIER / INTFOLD_FULLFAT are set, so the
# unpatched AlphaFold 3 behaviour is preserved. Upstream: google-deepmind/alphafold3.
# ----------------------------------------------------------------------------
...
from intellifold import patches as intellifold_patches
import haiku as hk
import jax
...
_WORK_QUEUE_DIR = flags.DEFINE_string(
    'work_queue_dir',
    None,
    'IntelliFold extension: directory for a local work queue. When set together'
    ' with --input_dir, this process becomes one worker in a single-machine pool:'
    ' it claims targets from the queue by atomic mkdir and skips ones already'
    ' done/failed, so one worker per GPU divides the directory with no overlap,'
    ' self-balancing, and a re-run resumes. Set automatically by'
    ' `intellifold predict --gpus`.',
)
_WORK_QUEUE_RETRY_FAILED = flags.DEFINE_bool(
    'work_queue_retry_failed',
    False,
    'When using --work_queue_dir, also re-attempt targets previously marked'
    ' .failed (default: skip them).',
)
...
_FLASH_ATTENTION_IMPLEMENTATION = flags.DEFINE_enum(
    ...
    help=(
        ...
        ' Triton and cuDNN flash attention implementation, respectively; both are'
        ' flash kernels and require Ampere GPUs or later. IntelliFold defaults to'
        " 'cudnn'. 'xla' uses an XLA attention implementation (no flash attention),"
        ' portable across GPU devices but materialises the N×N attention matrix, so'
        ' it can run out of memory on the largest complexes.'
    ),
)
...
def make_model_config(
    *,
    flash_attention_implementation: tokamax.DotProductAttentionImplementation = 'cudnn',
  ...
  # --- IntelliFold: make the stock AF3 network load IntelliFold-v2 weights. ---
  # Both calls are env-gated and no-op unless INTFOLD_FOURIER / INTFOLD_FULLFAT
...
def predict_structure(
  ...
  # IntelliFold: tolerate AF3 minor-version differences in featurise_input's signature
  # (e.g. v3.0.2 lacks `fix_standalone_glycans` / `resolve_msa_overlaps`). Pass only
  # kwargs the installed AF3 actually accepts.
  _feat_kwargs = dict(
  ...
  _accepted = _inspect.signature(featurisation.featurise_input).parameters
  featurised_examples = featurisation.featurise_input(
      **{k: v for k, v in _feat_kwargs.items() if k in _accepted}
  )
  print(
...
def main(_):
  ...
  # IntelliFold work-queue mode loads each target lazily as it is claimed, so we
  # do not eagerly enumerate the directory here (some MSAs are hundreds of MB).
  work_queue = None
  if _WORK_QUEUE_DIR.value is not None:
    if _INPUT_DIR.value is None:
      raise ValueError('--work_queue_dir requires --input_dir.')
    from intellifold import parallel
    work_queue = parallel.WorkQueue(
        _INPUT_DIR.value, _WORK_QUEUE_DIR.value,
        retry_failed=_WORK_QUEUE_RETRY_FAILED.value,
    )
    fold_inputs = None
  elif _INPUT_DIR.value is not None:
  ...
  if work_queue is not None:
    # Pool worker: keep claiming targets from the shared queue until it drains.
    # One target's failure (e.g. OOM) is recorded and skipped, never killing the
    # worker — so the rest of this GPU's share still gets processed.
    s = work_queue.stats()
    print(f'[queue] start: {s["remaining"]} remaining of {s["total"]} '
          f'({s["done"]} done, {s["failed"]} failed, {s["in_progress"]} in progress)')
    for stem, fold_input in work_queue.claim_iter(
        folding_input.load_fold_inputs_from_path):
      try:
        _process(fold_input)
        work_queue.mark_done(stem)
        num_fold_inputs += 1
      except KeyboardInterrupt:
        work_queue._release(stem)
        raise
      except Exception as e:  # noqa: BLE001
        print(f'[queue] FAILED {stem}: {type(e).__name__}: {e}')
        work_queue.mark_failed(stem, e)
    print(f'[queue] worker drained the queue; this worker did {num_fold_inputs} jobs.')
  else:
